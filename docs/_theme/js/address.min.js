const clickHandler=function(){let currentAddr=Cookies.get('address')||"https://127.0.0.1/";let addr=prompt("Set your HOPE server address",currentAddr);Cookies.set('address',addr,currentAddr);location.reload();};const setAddress=function(){let cookieAddr=Cookies.get('address');if(!cookieAddr){cookieAddr="[SERVER_ADDRESS]"}
...
function onReadyStateChangeReplacement(){if(this.readyState===XMLHttpRequest.DONE){setTimeout(setAddress,100);}
